...
class BasePlugin:
    ...
    def onStart(self):
        ...
        Options = { "Custom" : "1;V"} 
        if 4 not in Devices:
            Domoticz.Device(Name="L1 Current", Unit=4,TypeName="Custom",Used=0).Create()
        Options = { "Custom" : "1;A"} 
        if 5 not in Devices:
            Domoticz.Device(Name="L2 Current", Unit=5,TypeName="Custom",Used=0).Create()
        Options = { "Custom" : "1;A"} 
        if 6 not in Devices:
            Domoticz.Device(Name="L3 Current", Unit=6,TypeName="Custom",Used=0).Create()
        Options = { "Custom" : "1;A"} 
        if 7 not in Devices:
    ...
    def onHeartbeat(self):
        ...
        if self.runInterval <= 0:
            ...
            L1_Volts = self.rs485.read_float(1, functioncode=4, numberOfRegisters=2)
            L2_Volts = self.rs485.read_float(3, functioncode=4, numberOfRegisters=2)
            L3_Volts = self.rs485.read_float(5, functioncode=4, numberOfRegisters=2)
            L1_Current = self.rs485.read_float(7, functioncode=4, numberOfRegisters=2)
            L2_Current = self.rs485.read_float(9, functioncode=4, numberOfRegisters=2)
            L3_Current = self.rs485.read_float(11, functioncode=4, numberOfRegisters=2)
            L1_Active_Power = self.rs485.read_float(13, functioncode=4, numberOfRegisters=2)
            L2_Active_Power = self.rs485.read_float(15, functioncode=4, numberOfRegisters=2)
            L3_Active_Power = self.rs485.read_float(17, functioncode=4, numberOfRegisters=2)
