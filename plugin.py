...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 4 not in Devices:
            Domoticz.Device(Name="L1 Current", Unit=4,Type=0xF3,Subtype=0x23,Used=0).Create()
        Options = { "Custom" : "1;A"} 
        if 5 not in Devices:
            Domoticz.Device(Name="L2 Current", Unit=5,Type=0xF3,Subtype=0x23,Used=0).Create()
        Options = { "Custom" : "1;A"} 
        if 6 not in Devices:
            Domoticz.Device(Name="L3 Current", Unit=6,Type=0xF3,Subtype=0x23,Used=0).Create()
        Options = { "Custom" : "1;A"} 
    ...
    def onHeartbeat(self):
        ...
        if self.runInterval <= 0:
            ...
            #Devices[4].Update(0,str(Total_active_energy_kWh))
            #Devices[5].Update(0,str(Resettable_total_kWh))
            #Devices[6].Update(0,str(Resettable_import_kWh))
            #Devices[7].Update(0,str(Resettable_export_kWh))
